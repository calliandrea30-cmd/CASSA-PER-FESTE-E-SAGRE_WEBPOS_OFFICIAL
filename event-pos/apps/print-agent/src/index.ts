...
function getRawExePath(): string | null {
  if (cachedRawExePath !== undefined) return cachedRawExePath;
  const candidates = [
    path.join(__dirname, '../bin/raw-print.exe'),
    path.join(__dirname, 'bin/raw-print.exe'),
    path.join(process.cwd(), 'apps/print-agent/bin/raw-print.exe'),
    path.join(process.cwd(), 'bin/raw-print.exe'),
    // Quando gira come .exe pkg: accanto all'eseguibile principale
    path.join(path.dirname(process.execPath), 'bin/raw-print.exe'),
    path.join(path.dirname(process.execPath), 'raw-print.exe'),
  ];
  cachedRawExePath = candidates.find(p => { try { return fs.existsSync(p); } catch { return false; } }) || null;
  if (cachedRawExePath) {
...
function getAvailableSystemPrinters(): string[] {
  ...
  try {
    if (isWin) {
      ...
      try {
        const regOut = execSync('reg query "HKCU\\Software\\Microsoft\\Windows NT\\CurrentVersion\\Devices"', { encoding: 'utf8', timeout: 1000 });
        const lines = regOut.split(/\r?\n/).map(l => l.trim()).filter(Boolean);
        const list: string[] = [];
        for (const l of lines) {
          const match = l.match(/^([^\s].*?)\s+REG_SZ\s+/);
          if (match && match[1]) list.push(match[1].trim());
        }
      ...
      const out = execSync('lpstat -e 2>/dev/null || true', { encoding: 'utf8', timeout: 2000 });
      return out.split(/\r?\n/).map(p => p.trim()).filter(Boolean);
    }
  } catch {
    return [];
...
function getCachedPrinters(): string[] {
  const now = Date.now();
...
function getCachedDefaultPrinter(): string | null {
  const now = Date.now();
...
class SystemDefaultPrinterAdapter {
  ...
  close(callback?: (err?: any) => void) {
    ...
    if (isWin) {
      ...
      if (rawExe && targetP) {
        execFile(rawExe, [targetP, tmpFile], { timeout: 5000 }, (err) => {
          if (!err) {
      ...
      execFile('lpr', cupsArgs, { timeout: 8000 }, (err) => {
        if (err) {
 ...
 */
function sendViaPowerShellFallback(
  printerName: string,
  filePath: string,
  byteCount: number,
...
`.trim().replace(/\r?\n/g, ' ');
  ...
  execFile('powershell', ['-NoProfile', '-NonInteractive', '-Command', psInline], { timeout: 10000 }, (err) => {
    if (err) {
...
function applySize(printer: any, sizeStr: string) {
  ...
  else if (sizeStr === 'DOUBLE_HEIGHT') printer.size(0, 1);
  else printer.size(0, 0); // 0, 0 = 1x dimensione normale (NO ingrandimento 200%)
}
...
function getLineWidth(settings: any): number {
  return settings?.bodyFont === 'a' ? 32 : 40;
}

function makeDashLine(width: number): string {
  return '-'.repeat(width);
}

function makeEqLine(width: number): string {
  return '='.repeat(width);
}
...
function printComande(printer: any, payload: any, settings: any, categoryFilter?: string) {
  ...
  if (settings.comandaShowHeader === false) return;
