...
function getDefaultSystemPrinter() {
    ...
    try {
        if (isWin) {
            ...
            const out = (0, child_process_1.execSync)(cmd, { encoding: 'utf8', timeout: 3000 }).trim();
            return out || null;
        }
        else {
            // macOS / Linux CUPS
            const out = (0, child_process_1.execSync)('lpstat -d 2>/dev/null || true', { encoding: 'utf8', timeout: 3000 });
            const match = out.match(/:\s*([^\r\n]+)/);
            if (match && match[1]) {
                return match[1].trim();
            }
            // Fallback: cerca stampante contenente POS, 80 o Thermal in lpstat -p
            const pOut = (0, child_process_1.execSync)('lpstat -p 2>/dev/null || true', { encoding: 'utf8', timeout: 3000 });
        ...
        try {
            ...
            if (isWin) {
                ...
                const lpCmd = target
                    ...
                    : `lpr -l "${tmpFile}"`;
                (0, child_process_1.execSync)(lpCmd, { timeout: 8000 });
            }
...
function formatThreeColumns(col1, col2, col3, w1 = 7, w3 = 7, totalWidth = 40) {
...
function printItemRow(printer, qty, name, price, width = 40) {
    const w1 = 7;
    const w3 = 7;
...
function printScontrino(printer, payload, settings) {
    ...
    if (rawSubtitle) {
        ...
        rawSubtitle.split('\n').map((l) => l.trim()).filter(Boolean).slice(0, 2).forEach((line) => {
            printer.text(line);
        });
    }
        ...
        if (payload.type === 'REPORT_X' || payload.type === 'REPORT_Z') {
            ...
            await new Promise((resolve) => {
                printOnAdapter(printerConfig, (printer) => {
                    ...
                    printer.font('a').align('ct').style('b').size(1, 2).text(payload.type === 'REPORT_Z' ? 'CHIUSURA DI CASSA Z' : 'LETTURA X (RESOCONTO)');
