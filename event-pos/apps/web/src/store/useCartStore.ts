...
export const useCartStore = create<CartState>()(
  persist(
      ...
      appendNote: (id, note) => {
        set({
          items: get().items.map(i => {
            if (i.id !== id) return i;
            // Se l'item ha già una variante, aggiungi la nota al nome
            // Altrimenti crea una "variante" fittizia che porta la nota
            const v = i.variant
              ? { ...i.variant, name: `${i.variant.name} - ${note}` }
              : { id: 'note', productId: i.product.id, name: note, priceDelta: 0 };
    ...
    {
      name: 'sagrapos-cart',          // chiave in localStorage
      storage: createJSONStorage(() => localStorage),
      // Serializza solo i campi necessari
